...
bool cMap::onInteraction(std::vector<RPG_Dynamic *> &vecDynObjs,
                         RPG_Dynamic *target, cMap::NATURE nature) {
    return false;
}

void cMap::onChange(RPG_Dynamic *player) {}

cMap_Village::cMap_Village() { Create("../res/village.txt", "village"); }
...
bool cMap_Village::PopulateDynamics(std::vector<RPG_Dynamic *> &vecDynamics) {
    ...
    DynamicCreature *enemy =
        new DynamicCreatureEnemy(DynamicCreatureEnemy::MAN, "Gonzo");
    enemy->px = 16;
    enemy->py = 3;
    vecDynamics.push_back(enemy);
    DynamicCreature *enemy2 =
        new DynamicCreatureEnemy(DynamicCreatureEnemy::WOMAN, "Ginzi");
    enemy2->px = 10;
    enemy2->py = 10;
...
bool cMap_Village::onInteraction(std::vector<RPG_Dynamic *> &vecDynObjs,
                                 RPG_Dynamic *target, cMap::NATURE nature) {
    if (target->sName == "Teleport") {
        g_scriptProcessor->addCommand(new Command_ChangeMap(
            ((Teleport *)target)->targetMapName, ((Teleport *)target)->mapPosX,
...
void cMap_Village::onChange(RPG_Dynamic *player) {
    ...
    g_scriptProcessor->addCommand(new Command_ShowDialog(
        {"Welcome.", "Today you have a secret meeting with Gonzo",
         "And you have a plan to scam him"}));
    g_scriptProcessor->addCommand(
        new Command_ShowDialog({"Gonzo:", "Where is my stuff?"}));
    g_scriptProcessor->addCommand(
        new Command_ShowDialog({"You:", "Money first"}));
    g_scriptProcessor->addCommand(new Command_ShowDialog(
        {"Gonzo:", "Its here, take it. Now give me my stuff"}));
    g_scriptProcessor->addCommand(new Command_ShowDialog(
        {"You:", "I don't have the stuff, haha.", "Catch me if you can!"}));
    g_scriptProcessor->addCommand(new Command_ShowDialog(
        {"Run inside the building", "Don't let Gonzo catch you"},
        {0xFF, 0, 0}));
}
...
bool cMap_Home::onInteraction(std::vector<RPG_Dynamic *> &vecDynObjs,
                              RPG_Dynamic *target, cMap::NATURE nature) {
...
void cMap_Home::onChange(RPG_Dynamic *player) {
    g_scriptProcessor->addCommand(new Command_MoveTo(player, 5, 6, 2.0f));
    g_scriptProcessor->addCommand(new Command_ShowDialog(
        {"You have entered a closed balloon shop.",
         "Collect all the balloons in the shop and then exit"}));
}
